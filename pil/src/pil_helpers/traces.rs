...
pub const POSEIDON_2_AIR_IDS: &[usize] = &[13];

pub const SPECIFIED_RANGES_AIR_IDS: &[usize] = &[14];

pub const VIRTUAL_TABLE_0_AIR_IDS: &[usize] = &[15];

pub const VIRTUAL_TABLE_1_AIR_IDS: &[usize] = &[16];
...
pub type Poseidon2Fixed<F> = GenericTrace<Poseidon2FixedRow<F>, 131072, 0, 13>;

trace_row!(Poseidon2TraceRow<F> {
 in_use_clk_0:bit, in_use:bit, chunks:[[u32; 2]; 16], step_addr:ubit(40),
});
pub type Poseidon2Trace<F> = GenericTrace<Poseidon2TraceRow<F>, 131072, 0, 13>;


pub type Poseidon2TracePacked<F> = GenericTrace<Poseidon2TraceRowPacked<F>, 131072, 0, 13>;


trace_row!(SpecifiedRangesFixedRow<F> {
 RANGE: [F; 20], __L1__: F,
});
pub type SpecifiedRangesFixed<F> = GenericTrace<SpecifiedRangesFixedRow<F>, 1048576, 0, 14>;

trace_row!(SpecifiedRangesTraceRow<F> {
 mul:[F; 20],
});
pub type SpecifiedRangesTrace<F> = GenericTrace<SpecifiedRangesTraceRow<F>, 1048576, 0, 14>;


trace_row!(VirtualTable0FixedRow<F> {
 UID: [F; 7], column: [F; 39], __L1__: F,
});
pub type VirtualTable0Fixed<F> = GenericTrace<VirtualTable0FixedRow<F>, 2097152, 0, 15>;

trace_row!(VirtualTable0TraceRow<F> {
 multiplicity:[F; 7],
});
pub type VirtualTable0Trace<F> = GenericTrace<VirtualTable0TraceRow<F>, 2097152, 0, 15>;


trace_row!(VirtualTable1FixedRow<F> {
 UID: [F; 8], column: [F; 64], __L1__: F,
});
pub type VirtualTable1Fixed<F> = GenericTrace<VirtualTable1FixedRow<F>, 2097152, 0, 16>;

trace_row!(VirtualTable1TraceRow<F> {
 multiplicity:[F; 8],
});
pub type VirtualTable1Trace<F> = GenericTrace<VirtualTable1TraceRow<F>, 2097152, 0, 16>;
