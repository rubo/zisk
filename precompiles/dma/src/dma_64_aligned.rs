...
/// The `DmaSM` struct encapsulates the logic of the Dma64Aligned State Machine.
pub struct DmaSM<F: PrimeField64> {
    /// Reference to the PIL2 standard library.
    pub std: Arc<Std<F>>,

    /// Number of available Dma64Aligned in the trace.
...
impl<F: PrimeField64> DmaSM<F> {
    ...
    ///
    /// # Arguments
    /// * `trace` - A mutable reference to the Dma64Aligned trace.
    ...
    pub fn compute_witness(
        ...
        tracing::info!(
            "··· Creating Dma64Aligned instance [{} / {} rows filled {:.2}%]",
            total_inputs,
            num_rows,
        ...
        // Split the dma64aligned_trace.buffer into slices matching each inner vector’s length.
        ...
        trace.row_slice_mut()[total_inputs..num_rows]
            .par_iter_mut()
            .for_each(|slot| *slot = Dma64AlignedTraceRow::<F> { ..Default::default() });
