...
use zisk_pil::{DmaPrePostTrace, DmaPrePostTraceRow};
...
pub struct DmaPrePostSM<F: PrimeField64> {
    ...
    /// Number of available dmapreposts in the trace.
...
impl<F: PrimeField64> DmaPrePostSM<F> {
    ...
    ///
    /// # Arguments
    /// * `trace` - A mutable reference to the DmaPrePost trace.
    ...
    pub fn compute_witness(
        ...
        tracing::info!(
            "··· Creating DmaPrePost instance [{} / {} rows filled {:.2}%]",
            total_inputs,
            num_rows,
            total_inputs as f64 / num_rows as f64 * 100.0
        );

        timer_start_trace!(DMAPREPOST_TRACE);

        // Split the dmaprepost_trace.buffer into slices matching each inner vector’s length.
        ...
        timer_stop_and_log_trace!(DMAPREPOST_TRACE);

        trace.row_slice_mut()[total_inputs..num_rows]
            .par_iter_mut()
            .for_each(|slot| *slot = DmaPrePostTraceRow::<F> { ..Default::default() });
