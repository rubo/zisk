...
#[derive(Debug)]
#[repr(C)]
pub struct SyscallBls12_381ComplexMulParams<'a> {
    pub f1: &'a mut SyscallComplex384,
    pub f2: &'a SyscallComplex384,
}

/// Performs the multiplication of two complex field elements on a complex extension of the Bls12_381 base field curve,
/// storing the result in the first field element.
///
/// The `Bls12_381ComplexMul` system call executes a CSR set on a custom port. When transpiling from RISC-V to Zisk,
/// this instruction is replaced with a precompiled operation—specifically, `Bls12_381ComplexMul`.
///
/// `Bls12_381ComplexMul` operates on two field elements, each with two coordinates of 384 bits.
